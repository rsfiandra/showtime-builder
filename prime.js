...
const PRIME_INPUT_BASE_CLASS = 'border border-gray-300 rounded px-2 py-1 w-20 text-center font-mono tabular-nums bg-white text-gray-800';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function focusPrimeInputByRowId(rowId) {
    if (!body) return;
    const inputs = body.querySelectorAll('[data-prime-entry="true"]');
  ...
  function normalisePrimeEntry(value) {
    if (typeof value !== 'string') return '';
    let v = value.trim().toLowerCase();
    if (!v) return '';
    const suffixMatch = v.match(/(am|pm|a|p)$/);
    let suffix = '';
    if (suffixMatch) {
  ...
  function movePrimeFocusFromElement(element, delta) {
    if (!body || !element) return;
    const inputs = Array.from(body.querySelectorAll('[data-prime-entry="true"]'));
  ...
  function render() {
    ...
    rows.forEach((row, idx) => {
      ...
      // Prime time cell: use a free-form text input so schedulers can type any time.
      // Duplicate times are still highlighted to make conflicts obvious.
      const tdPrime = document.createElement('td');
      const inputPrime = document.createElement('input');
      inputPrime.type = 'text';
      inputPrime.autocomplete = 'off';
      inputPrime.spellcheck = false;
      inputPrime.inputMode = 'numeric';
      inputPrime.placeholder = 'HH:MM';
      inputPrime.value = row.primeHM || '';
      inputPrime.dataset.rowId = row.rowId;
      inputPrime.dataset.primeEntry = 'true';

      function updatePrimeInputState() {
        inputPrime.className = PRIME_INPUT_BASE_CLASS;
        const raw = inputPrime.value;
        const trimmed = raw.trim();
        const normalised = normalisePrimeEntry(raw);
        const hasValue = trimmed.length > 0;
        const isInvalid = hasValue && !normalised;
        const isDuplicate = !isInvalid && normalised && primeCounts[normalised] && (row.primeHM !== normalised || primeCounts[normalised] > 1);
        if (isInvalid) {
          inputPrime.classList.remove('border-gray-300');
          inputPrime.classList.add('border-amber-500', 'bg-amber-50', 'text-amber-700');
        } else if (isDuplicate) {
          inputPrime.classList.remove('border-gray-300');
          inputPrime.classList.add('border-red-500', 'bg-red-50', 'text-red-700');
        }
      }

      function commitPrimeValue() {
        const raw = inputPrime.value;
        const trimmed = raw.trim();
        const normalised = normalisePrimeEntry(raw);
        if (trimmed && !normalised) {
          inputPrime.value = row.primeHM || '';
          updatePrimeInputState();
          return;
        }
        const finalValue = normalised || '';
        if (finalValue === row.primeHM) {
          inputPrime.value = finalValue;
          updatePrimeInputState();
          return;
        }
        ShowtimeState.setRowField(row.rowId, 'primeHM', finalValue);
        render();
        requestAnimationFrame(() => focusPrimeInputByRowId(row.rowId));
      }

      inputPrime.addEventListener('focus', () => {
        updatePrimeInputState();
        inputPrime.select();
      });
      inputPrime.addEventListener('input', updatePrimeInputState);
      inputPrime.addEventListener('blur', commitPrimeValue);
      inputPrime.addEventListener('keydown', evt => {
        if (evt.key === 'Enter') {
          evt.preventDefault();
          commitPrimeValue();
          ...
          movePrimeFocusFromElement(inputPrime, delta);
        }
      });
      ...
      tdPrime.appendChild(inputPrime);
